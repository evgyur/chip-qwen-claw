...
Если секция `agents.defaults.models` уже есть — добавить только ключ `"groq/qwen/qwen3-32b"` с `"alias": "qwen"`, не затирая остальные модели.

- **Важно:** Groq API ожидает id модели **`qwen/qwen3-32b`**. В OpenClaw ссылка поэтому должна быть **`groq/qwen/qwen3-32b`** (провайдер `groq` + id `qwen/qwen3-32b`). Использование `groq/qwen3-32b` приводит к 404 «The model qwen3-32b does not exist».
...
2. Идентификатор модели тогда будет **`groq/qwen/qwen3-32b`** (провайдер + id). В `agents.defaults.models` задать `"groq/qwen/qwen3-32b": { "alias": "qwen" }`.
...
2. Добавить в конфиг OpenClaw в `agents.defaults.models`: `"groq/qwen/qwen3-32b": { "alias": "qwen" }`.
